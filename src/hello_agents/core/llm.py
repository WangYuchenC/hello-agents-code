...
class HelloAgentsLLM:
    def __init__(
        ...
        **kwargs
        ...
        Args:
            model (str | None, optional): 模型名称，如果未提供则从环境变量LLM_MODEL_ID读取.
            api_key (str | None, optional): API密钥，如果未提供则从环境变量读取.
            base_url (str | None, optional): 服务地址，如果未提供则从环境变量LLM_BASE_URL读取.
            provider (SUPPORTED_PROVIDERS | None, optional): LLM提供商，如果未提供则自动检测.
            temperature (float, optional): 温度参数.
            max_tokens (int | None, optional): 最大token数.
            timeout (int | None, optional): 超时时间，从环境变量LLM_TIMEOUT读取，默认60秒.
        ...
        if not all([self.api_key, self.base_url]):
            raise ValueError("API密钥和服务地址必须提供或在.env文件中定义")
    ...
    def _auto_detect_provider(self, api_key: str | None, base_url: str | None) -> str:
        ...
        if actual_api_key:
            actual_api_key = actual_api_key.lower()
            if actual_api_key.startswith("ms-"):
                return "modelscope"
            elif actual_api_key == "ollama":
                return "ollama"
            elif actual_api_key == "vllm":
                return "vllm"
            elif actual_api_key == "local":
                return "local"
    ...
    def think(
        ...
        Args:
            ...
            temperature (float | None, optional): 温度参数，如果未提供，则使用初始化时的值。
